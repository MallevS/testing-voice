...
export default function BulkUploadNumbers({ onClose, onCall }: BulkUploadNumbersProps) {
  ...
  // Emit statistics whenever contacts change
  useEffect(() => {
    emitStatistics();
  }, [contacts, isCalling]);
  ...
  const emitStatistics = () => {
    const stats = {
      ...
      failed: contacts.filter(c => ['failed', 'busy', 'no-answer'].includes(c.status)).length,
      inProgress: contacts.filter(c => ['calling', 'ringing', 'in-progress'].includes(c.status)).length,
      pending: contacts.filter(c => ['pending', 'saved'].includes(c.status)).length,
      successRate: 0,
      averageCallTime: 0
    };
    ...
    if (totalAttempts > 0) {
      stats.successRate = (stats.completed / totalAttempts) * 100;
    }

    const completedCalls = contacts.filter(c => c.status === 'completed' && c.callDuration);
    if (completedCalls.length > 0) {
      const totalDuration = completedCalls.reduce((sum, c) => sum + (c.callDuration || 0), 0);
      stats.averageCallTime = totalDuration / completedCalls.length;
    }

    // Dispatch custom event
    const event = new CustomEvent('callStatsUpdate', { detail: stats });
  ...
  const handleSaveContacts = async () => {
    ...
    try {
      const savePromises = contacts.map(async (contact) => {
        ...
        return { ...contact, firestoreId: docRef.id, status: 'pending' as const };
      });
  ...
  const pollCallStatus = async (callSid: string, contactIdx: number, startTime: number): Promise<string> => {
    const maxAttempts = 180;
    let attempts = 0;
    ...
    while (attempts < maxAttempts) {
      try {
        ...
        if (['completed', 'failed', 'busy', 'no-answer', 'canceled'].includes(status)) {
          ...
          setContacts(prev =>
            prev.map((c, i) =>
              i === contactIdx ? { ...c, callDuration: duration } : c
            )
          );
          return status;
        }
  ...
  const handleCallSelected = async () => {
    ...
    for (const idx of selectedIndexes) {
      ...
      setContacts(prev =>
        prev.map((c, i) => i === idx ? { ...c, status: 'calling', callStartTime } : c)
      );
      ...
      try {
        ...
        const finalStatus = await pollCallStatus(callSid, idx, callStartTime);
  ...
  const handleRetryFailed = async () => {
    const failedIndexes = contacts
      .map((c, i) => ['failed', 'busy', 'no-answer'].includes(c.status) ? i : -1)
      .filter(i => i !== -1);

    if (failedIndexes.length === 0) {
      alert("No failed calls to retry");
      return;
    }

    setSelectedContacts(new Set(failedIndexes));
    // The user can now click "Call Selected" or we could auto-trigger
  };
  ...
  const failedCount = contacts.filter(c => ['failed', 'busy', 'no-answer'].includes(c.status)).length;
            ...
            <table className="w-full text-sm">
              <thead className="bg-gray-100 sticky top-0">
                <tr>
                  ...
                  <th className="px-2 py-2 text-left text-black">Email</th>
                  <th className="px-2 py-2 text-left text-black">Status</th>
                  <th className="px-2 py-2 text-left text-black">Duration</th>
                </tr>
              </thead>
              <tbody>
                {contacts.map((contact, idx) => (
                  <tr key={idx} className="border-b hover:bg-gray-50">
                    ...
                    <td className="px-2 py-2 text-xs text-gray-600">
                      {contact.callDuration ? `${contact.callDuration}s` : '—'}
                    </td>
                  </tr>
                ))}
              </tbody>
